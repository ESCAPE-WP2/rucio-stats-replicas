...
def get_replicas(push_to_es=False, es_url=None):
	...
	for scope in scopes_list:
		...
		for rse in rses_list:
			...
			# Protected Replicas:
			tRules = session.execute("SELECT COUNT(*) FROM rules ru INNER JOIN replicas rep ON ru.scope=rep.scope AND ru.name=rep.name INNER JOIN rses r ON rep.rse_id=r.id AND rep.state='A' AND ru.state='O' AND ru.scope='"+scope+"' AND r.rse='"+rse+"'").fetchone()
			tRules_bytes = session.execute("SELECT SUM(rep.bytes) FROM replicas rep INNER JOIN rules ru ON ru.scope=rep.scope AND ru.name=rep.name INNER JOIN rses r ON rep.rse_id=r.id AND rep.state='A' AND rep.scope='"+scope+"' AND r.rse='"+rse+"'").fetchone()
